...
! MO, 07/05/23: added z_inv_glob
   real tmp, grad, grad_max(nx,ny), z_inv_mean
   integer z_inv_ind(nx,ny)
   ...
   grad_max(:,:) = 0.  ! MO, 07/05/23: added z_inv_glob
   z_inv_ind(:,:) = 1
   z_inv_glob = 0.
   ...
   do k=1,nzm
      ...
      tmp=1./(z(kc)-z(kb))  ! MO, 07/05/23: added z_inv_glob
      ...
      do j=1,ny
         do i=1,nx
            ...
            if(z(k).lt.4000.) then ! shallow clouds only
               ...
               grad = (t(i,j,kc)-t(i,j,kb))*tmp
               if(grad_max(i,j).lt.grad)then
                  grad_max(i,j) = grad
                  z_inv_ind(i,j)=k
               end if
            end if
            ! end MO, 7/05/23: added z_inv_glob
         end do
      end do
      u0(k)=u0(k)*coef
      v0(k)=v0(k)*coef
...
! MO, 07/05/23: added z_inv_glob
   do j=1,ny
      do i=1,nx
         z_inv_mean = z_inv_mean + z(z_inv_ind(i,j))
      end do
   end do
   z_inv_mean = z_inv_mean * coef
! end MO, 7/05/23: added z_inv_glob
   ...
   if(dompi) then
      ...
      ! MO, 07/05/23: added z_inv_glob
      call task_sum_real8(z_inv_mean,z_inv_glob,1)
      z_inv_glob = z_inv_glob * coef1
      ! end MO, 7/05/23: added z_inv_glob
