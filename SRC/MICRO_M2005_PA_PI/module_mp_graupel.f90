...
! FOR INUM = 1, SET CONSTANT DROPLET CONCENTRATION (CM-3)
   REAL, PRIVATE ::      NDCNST
   real, public ::   arctic_ice_nuclei_Nice, cjhet, mjhet
...
!      = 2, pwu COPMBLE-MIP, PA_PI (ABIFM, Knopf et al. 2023 JAMES)
...
! pwu COMBLE-MIP, ABIFM ice formation
      REAL, DIMENSION(KMS:KME) ::   a_ice_w  ! water activity at freezing temperature
      REAL, DIMENSION(KMS:KME) ::   jhet     ! heteogeneous freezing coefficient
      REAL, DIMENSION(KMS:KME) ::   aero_area ! total aerosol surface area
...
! RATE OF CHANGE SAT. MIX. RATIO WITH TEMPERATURE
         ...
         ! pwu moved the following block from within IF (T3D(K).GE.TMELT)
...
! USE MINIMUM VALUE OF 1.E-6 TO PREVENT FLOATING POINT ERROR
           ...
           IF (QC3D(K).GE.1.E-6) THEN
            ! IF (QC3D(K).GE.1.E-5.and.nc3d(k).ge.1e2) THEN ! pwu 2025-06-27: avoid large PRC in very low Nc conditions
...
!               DBGNAD(1) = DBGNAD(1) + NAD3D(K)
            ...
            ! pwu moved the following block to before if temperature>freezing
            ! IF (IPRGAER.EQ.1) THEN
            !    NANEW1 = (NAD3D(K) + NC3D(K))*RHO(K)
            !    NANEW2 = 0. !Placeholder for further dry aerosol modes !brnr
            !    MAER1 = (QAD3D(K) + QAW3D(K))*RHO(K)
...
! add threshold according to Greg Thomspon
            ...
            ELSE IF (INUC.EQ.1) THEN
               ...
               if (T3D(K).LT.268.15.and.(qc3d(k)+qr3d(k)).ge.0.001e-3) then ! pwu COMBLE-MIP

                  !KC2 = 0.16*1000./RHO(K)  ! CONVERT FROM L-1 TO KG-1
                  KC2 = arctic_ice_nuclei_Nice*1000./RHO(K)  ! CONVERT FROM L-1 TO KG-1
                  IF (KC2.GT.NI3D(K)+NS3D(K)+NG3D(K)) THEN
                     NNUCCD(K) = (KC2-NI3D(K)-NS3D(K)-NG3D(K))/DT
                     MNUCCD(K) = NNUCCD(K)*MI0
                  END IF
            ...
            ELSE IF (INUC.EQ.2) THEN
               !  if (T3D(K).LT.268.15.and.(qc3d(k)+qr3d(k)).ge.0.001e-3) then
               if (T3D(K).LT.268.15) then

                  ! Koop & Zobrist (2009), accurate between 273.15 K and 173 K
                  a_ice_w(k) = exp(9.550426 - 5723.265 / T3D(K) + 3.53068 * log(T3D(K)) - &
                     0.00728332 * T3D(K)) /                               &
                     (exp(54.842763 - 6763.22 / T3D(K) -                         &
                     4.210 * log(T3D(K)) + 0.000367 * T3D(K) +                    &
                     tanh(0.0415 * (T3D(K) - 218.8)) *                          &
                     (53.878 - 1331.22 / T3D(K) - 9.44523 * log(T3D(K)) + 0.014025 * T3D(K))))

                  jhet(k) = 10.**(cjhet + mjhet * (QVQVS(K) - a_ice_w(k))) * 1e4  * 25. ! 25jhet, m-2 s-1

                  ! half of accumulation mode aerosol as SSA (INP)
                  KC2 = (NAD3D(K) + NC3D(K))*RHO(K)/2   ! kg-1 to m-3
                  aero_area(k) = KC2 * pi * (2.*RM1)**2 * exp(2*log(SIG1)**2) ! not really surface area, m-1

                  NNUCCD(K) = aero_area(k) * jhet(k)/RHO(k) ! m-1 m-2 s-1 m3 kg-1 = kg-1 s-1
                  MNUCCD(K) = NNUCCD(K)*MI0
               end if
            END IF ! IF (INUC)
...
! MAKE SURE THIS VALUE ISN'T GREATER THAN TOTAL NUMBER OF AEROSOL
                           ...
                           DUM2 = (DUM2-NC3D(K))/DT
                           DUM2 = MAX(0.,DUM2)
                           NACT(K) = DUM2 !brnr track aerosol activation tendency ! actually executed COMBLE-MIP
                           NC3DTEN(K) = NC3DTEN(K)+DUM2
