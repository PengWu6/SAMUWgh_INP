...
real tkedissipation    (nx, ny, nzm)      ! tke dissipation rate, pwu COMBLE-MIP 02/19/2024
real tkedissipations   (nx, ny, nzm)      ! tke dissipation rate - sgs, pwu COMBLE-MIP 02/19/2024
real vmlc  (nx, ny, nzm) !mass_weighted_fall_speed_of_liquid_cloud_water
real vmlr  (nx, ny, nzm) !mass_weighted_fall_speed_of_rain
real vmic  (nx, ny, nzm) !mass_weighted_fall_speed_of_ice_cloud
real vmis  (nx, ny, nzm) !mass_weighted_fall_speed_of_snow
real vmig  (nx, ny, nzm) !mass_weighted_fall_speed_of_graupel
!--------------------------------------------------------------------
...
real, dimension(nx,ny) :: fluxbu = 0., fluxbv = 0., fluxbt = 0.
real, dimension(nx,ny) :: fluxbq = 0., fluxtu = 0., fluxtv = 0.
real, dimension(nx,ny) :: fluxtt = 0., fluxtq = 0., fzero  = 0.
real, dimension(nx,ny) :: precsfc = 0. ! surface precip. rate
...
real, dimension(nx,ny) :: prec_xy = 0. ! mean precip. rate for outout
real, dimension(nx,ny) :: shf_xy = 0. ! mean precip. rate for outout
real, dimension(nx,ny) :: lhf_xy = 0. ! mean precip. rate for outout
real, dimension(nx,ny) :: lwns_xy = 0. ! mean net lw at SFC
real ustar_xy(nx,ny) ! ustar, pwu COMBL-MIP 02/25/2024
real rlmo_xy(nx,ny) ! reciprocal of Obukhov length,pwu COMBL-MIP 02/25/2024
real z0m(nx,ny) ! momrntum roughness length, pwu COMBLE-MIP 03/28/2024
real, dimension(nx,ny) :: swns_xy = 0. ! mean net sw at SFC
real, dimension(nx,ny) :: lwnsc_xy = 0. ! clear-sky mean net lw at SFC
real, dimension(nx,ny) :: swnsc_xy = 0. ! clear-sky mean net sw at SFC
real, dimension(nx,ny) :: lwnt_xy = 0. ! mean net lw at TOA
real, dimension(nx,ny) :: swnt_xy = 0. ! mean net sw at TOA
real, dimension(nx,ny) :: lwntc_xy = 0. ! clear-sky mean net lw at TOA
real, dimension(nx,ny) :: swntc_xy = 0. ! clear-sky mean net sw at TOA
real, dimension(nx,ny) :: solin_xy = 0. ! solar TOA insolation
real, dimension(nx,ny) :: pw_xy = 0.   ! precipitable water
real, dimension(nx,ny) :: cw_xy = 0.   ! cloud water path
real, dimension(nx,ny) :: iw_xy = 0.   ! ice water path
real, dimension(nx,ny) :: cld_xy = 0.   ! cloud frequency
real, dimension(nx,ny) :: u200_xy = 0. ! u-wind at 200 mb
real, dimension(nx,ny) :: usfc_xy = 0. ! u-wind at at the surface
real, dimension(nx,ny) :: v200_xy = 0. ! v-wind at 200 mb
real, dimension(nx,ny) :: vsfc_xy = 0. ! v-wind at the surface
real, dimension(nx,ny) :: w500_xy = 0. ! w at 500 mb
real, dimension(nx,ny) :: qocean_xy = 0. ! ocean cooling in W/m2
...
! pwu 02/14/2024 COMBLE-MIP
     real ustar_save, lmo_save
     real precfluxi(nz)
...
real, dimension(nx,ny) :: u850_xy = 0. ! zonal velocity at 850 mb
real, dimension(nx,ny) :: v850_xy = 0. ! meridional velocity at 850 mb

! Surface pressure
real, dimension(nx,ny) :: psfc_xy = 0. ! pressure (in millibar) at lowest grid point

! Saturated water vapor path, useful for computing column relative humidity
real, dimension(nx,ny) :: swvp_xy = 0.  ! saturated water vapor path (wrt water)

! Cloud and echo top heights, and cloud top temperature (instantaneous)
real, dimension(nx,ny) :: cloudtopheight = 0., echotopheight =0., cloudtoptemp = 0.
...
real z_inv_glob   ! Global-domain-mean inversion height 
                 ! (for altitude-varying nudging)
