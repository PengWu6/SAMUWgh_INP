...
! - Two mode (Aitken, accumulation) mode aerosol implementation by Matt Wyant, UW, ~2019-2022
!   Published in Wyant et al (2022, JAMES, https://doi.org/10.1029/2021MS002930).
!   includes simple implementation of sulfur chemistry, aerosol coagulation and scavenging
!   of unactivated aerosols is now based on the swollen diameter due to water vapor uptake.
...
!   aerosols and Aitken mode aerosols are transferred to the accumulation mode when the critical
!   diameter falls below the diameter at which the Aitken and accumulation mode distributions
!   cross.
...
use vars, only: pres, rho, dtn, w, t, tabs, qv, qcl, qpl, qci, qpi, &
     ...
     fluxbq, fluxtq, dtfactor, u10arr, precsfc, prec_xy, &
...
real, allocatable, dimension(:,:,:) :: NARG1, NARG2, NACTRATE, QACTRATE, NACTDIFF, NATRANS, QATRANS, ISACT, DC1, DC2, DG1, DG2 , DG1ACT, DG2ACT, SSPK, NCNEGLIM, NCPOSLIM
...
subroutine micro_setparm()
  ...
  integer :: count
   ...
   NAMELIST /MICRO_M2005_MA/ &
      ...
      donudging_aerosol, &  ! Enable nudging of aerosol
...
!bloss      do_micro3dout         ! flag to output 3d snapshot for a bunch of micro tendencies
  ...
  if(doprogaerosol.AND.doicemicro) then
  ...
  if(ABS(massfactor-1.).gt.4*EPSILON(1.)) then
     if(masterproc) write(*,*) 'massfactor = ', massfactor
     if(masterproc) write(*,*) 'M2005_MA has not been coded to account for values of massfactor that are not one.'
     if(masterproc) write(*,*) 'If massfactor>1 is required because of flux correction or positivity preservation, '
     if(masterproc) write(*,*) 'do careful testing to verify consistency of various tendencies (chemistry, '
     if(masterproc) write(*,*) 'coagulation, water uptake, scavenging, etc.) between runs with massfactor=1 and'
     if(masterproc) write(*,*) 'massfactor>1.'
     if(masterproc) write(*,*) '.... Stopping now ....'
     call task_abort()
  end if
  ...
  if(.not.isallocatedMICRO) then
     ...
     allocate(micro_field(dimx1_s:dimx2_s,dimy1_s:dimy2_s,nzm,nmicro_fields), &
          ...
          mktend_vadv(nzm,nmicro_fields), & !bloss(2023-01)
     ...
     if (doactivdiagoutput) then
            ...
            NATRANS(nx,ny,nzm), QATRANS(nx,ny,nzm), ISACT(nx,ny,nzm), DC1(nx,ny,nzm), DC2(nx,ny,nzm), DG1(nx,ny,nzm), DG2(nx,ny,nzm), DG1ACT(nx,ny,nzm), DG2ACT(nx,ny,nzm), SSPK(nx,ny,nzm), &
     ...
     if(doactivdiagoutput) then
       ! initialize microphysics 3D outputs
       NARG1(:,:,:) = 0.
       NARG2(:,:,:) = 0.
       NACTRATE(:,:,:) = 0.
       QACTRATE(:,:,:) = 0.
...
826    format('Initial inversion height is ',F8.1, &
       ...
       case(0)
         ...
         if(masterproc) then
         write(*,*) 'In MICRO_M2005_MA, using aerinitmode==0.  This means that '
...
!           micro_field(:,:,k,iqad) = dumM1 * micro_field(:,:,k,inad) / aer_n1
!           micro_field(:,:,k,iqaw) = dumM1 * micro_field(:,:,k,incl) / aer_n1
!         end do
...
!           micro_field(:,:,k,iqad) = dumM2 * micro_field(:,:,k,inad) / aer_n2
!           micro_field(:,:,k,iqaw) = dumM2 * micro_field(:,:,k,incl) / aer_n2
!         end do
...
!!$       dumM2 = (POCperturb_Na)*RHOA*pi/6.*(aer_rm1**3)*EXP(9.*(log(aer_sig1)**2)/2.) 
     ...
     ! Only apply massfactor scaling after complete initialization of aerosol mass
     micro_field(:,:,:,iqad) = micro_field(:,:,:,iqad)*massfactor ! accumulation mode mass mixing ratio
     micro_field(:,:,:,iqaw) = micro_field(:,:,:,iqaw)*massfactor ! in-cloud-droplet aerosol mass mixing ratio
     micro_field(:,:,:,iqar) = micro_field(:,:,:,iqar)*massfactor ! in-rain-droplet aerosol mass mixing ratio
     micro_field(:,:,:,iqad2) = micro_field(:,:,:,iqad2)*massfactor ! Aitken mode mass mixing ratio
 ...
 if(doprogaerosol) then
    mkname(iqad) = 'QAC'
    mklongname(iqad) = 'DRY AEROSOL MASS ACCUMULATION MODE'
    mkunits(iqad) = 'g/kg'
    mkoutputscale(iqad) = 1.e3/massfactor
    
    mkname(iqad2) = 'QAK'
    mklongname(iqad2) = 'DRY AEROSOL MASS AITKEN MODE'
    mkunits(iqad2) = 'g/kg'
    mkoutputscale(iqad2) = 1.e3/massfactor
    
    mkname(inad) = 'NAC'
    mklongname(inad) = 'DRY AEROSOL NUMBER CONCENTRATION ACCUMULATION MODE'
    mkunits(inad) = '#/cm3'
    mkoutputscale(inad) = 1.e-6
    
    mkname(inad2) = 'NAK'
    mklongname(inad2) = 'DRY AEROSOL NUMBER CONCENTRATION AITKEN MODE'
    mkunits(inad2) = '#/cm3'
    mkoutputscale(inad2) = 1.e-6
    ...
    mkname(iDMS) = 'DMS'
    mklongname(iDMS) = 'DMS GAS CONCENTRATION'
    mkunits(iDMS) = 'kg/kg'
    mkoutputscale(iDMS) = 1.

    mkname(iSO2) = 'SO2'
    mklongname(iSO2) = 'SO2 GAS CONCENTRATION'
    mkunits(iSO2) = 'kg/kg'
    mkoutputscale(iSO2) = 1.

    mkname(iH2SO4) = 'SO4'
    mklongname(iH2SO4) = 'H2SO4 GAS CONCENTRATION'
    mkunits(iH2SO4) = 'kg/kg'
    mkoutputscale(iH2SO4) = 1.

!    mkname(iH2O2) = 'PER'
!    mklongname(iH2O2) = 'H2O2 GAS CONCENTRATION'
!    mkunits(iH2O2) = 'kg/kg'
!    mkoutputscale(iH2O2) = 1.
...
subroutine micro_finalize()
  ...
  if(isallocatedMICRO) then
    ...
    deallocate(micro_field, fluxbmk, fluxtmk, &
       ...
       if (doactivdiagoutput) then
          deallocate(NARG1, NARG2, NACTRATE, QACTRATE, NACTDIFF, ISACT, DC1, DC2, DG1, DG2, DG1ACT, DG2ACT, SSPK, NATRANS, QATRANS, NCPOSLIM, NCNEGLIM)
       end if   
     if(ierr.ne.0) then
...
if(doprogaerosol) then
  select case(aerfluxmode)
    case(1) !fixed flux
      fluxbmk(:,:,iqad) = fluxQAd0*(1/RHOW(1))
    ...
    !bloss: Scale surface fluxes of aerosol mass mixing ratios by massfactor
    fluxbmk(:,:,iqad) = massfactor*fluxbmk(:,:,iqad)
    fluxbmk(:,:,iqad2) = massfactor*fluxbmk(:,:,iqad2)
...
real :: ddry
real :: sqrt2, xx, maerofrac, naerofrac
real :: massdryfrac, numberdryfrac, total_acc_number, total_acc_mass
...
!print*, '+++++++START of micro_proc'
!call micro_where_negative(iqad)
...
if(.NOT.doprogaerosol) STOP 'assuming doprogaerosol==.TRUE. in the code below'

micro_field(:,:,:,iqaw) = 0.

! move cloud water aerosol into qaw temporarily for upcoming physics
! compute mass fraction of accumulation aerosol that is dry (qad) and in cloud (qaw)
do i = 1,nx
   do j = 1,ny
      do k = 1,nzm
         if (micro_field(i,j,k,inad).gt.0.1) then
            total_acc_number = micro_field(i,j,k,inad) + micro_field(i,j,k,incl)
            total_acc_mass = micro_field(i,j,k,iqad) ! outside microphysics, all aerosol mass is in qad
            numberdryfrac = micro_field(i,j,k,inad)/total_acc_number
            massdryfrac = mass_fraction(numberdryfrac, sigma_accum)            

            ! within microphysics, distribute mass between activated (iqaw) and unactivated (iqad)
            micro_field(i,j,k,iqaw) = (1.-massdryfrac) * total_acc_mass
            micro_field(i,j,k,iqad) = massdryfrac * total_acc_mass
...
! to make these routines behave similarly with dototalwater=T/F, store cloud liquid
!    in cloudliq(:,:,:)
if(.NOT.dototalwater) cloudliq(1:nx,1:ny,1:nzm) = micro_field(1:nx,1:ny,1:nzm,iqcl)
...
if (doH2SO4uptake) then
   ...
   call gas_uptake_tendencies(micro_field(1:nx,1:ny,1:nzm,iH2SO4), micro_field(1:nx,1:ny,1:nzm,incl), &
        ...
        micro_field(1:nx,1:ny,1:nzm,iqad2), sigma_accum, sigma_aitken, cloudliq(1:nx,1:ny,1:nzm), tabs, &
...
!add qaw back into qad field (qaw used as temporary holding above) 
micro_field(:,:,:,iqad) = micro_field(:,:,:,iqad) + micro_field(:,:,:,iqaw)
micro_field(:,:,:,iqaw) = 0.
...
!      tmpw = 0.5*(w(i,j,1:nzm) + w(i,j,2:nz))  ! MK: changed for stretched grids 
      ...
      do k = 1,nzm
         if (tmpnad(k).gt.0.1) then
            numberdryfrac = tmpnad(k)/(tmpnad(k) + tmpncl(k))
            massdryfrac = mass_fraction(numberdryfrac, sigma_accum)
            tmpqaw(k) = (1.-massdryfrac) * tmpqad(k)
            tmpqad(k) = massdryfrac * tmpqad(k)    
          else   
            tmpqaw(k) = 0.
...
!print*, '####After module_mp_graupel and scavenging'      
!call micro_where_negative(iqad)
...
!bloss print*, 'XXXX End of micro_proc'
call micro_where_negative(iqad)
...
do n = 1,nmicro_fields
  if (flag_number(n).eq.1) then
     do k = 1,nzm
       mk0(k,n) = SUM(micro_field(1:nx,1:ny,k,n))/real(nx*ny) ! only multiply
               ! by rho on output.
     end do
  else
     do k = 1,nzm
...
do n = 1,nmicro_fields
   ...
   if(flag_number(n).eq.1) then
      ...
      mkstor(1:nzm,n) = mkstor(1:nzm,n)*rho(:)
   end if
