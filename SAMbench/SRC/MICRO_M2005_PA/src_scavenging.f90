...
! brnr check thermal conductivity value above, look up value for SO2
  ...
  !bloss(2020-10): threshold water mixing ratio for scavenging to be computed
  !   Move to be a global parameter so that parent routines can check to see
  !   if scavenging needs to be called.
...
!!$     REAL(KIND=dp)     :: sigma  !
  ...
  LOGICAL, PARAMETER :: LimitScavengingEfficiency = .false. ! If true, e_c, e_r <=1.
  LOGICAL, PARAMETER :: IncludeThermoDiffusophoresis = .false. 
          ...
          DO k=1,nt
             ...
             DO j=1,nc
                ...
                DO i=1,na
                   ...
                   if(IncludeThermoDiffusophoresis) then
                     !bloss(2020-11): Fix from Matt Wyant: Sum of thermo+hydrophoresis 
                     !                   should be positive
                   ...
                   if(LimitScavengingEfficiency) then
       ...
       DO k=1,nt
          ...
          DO j=1,nr
             ...
             DO i=1,na
                ...
                ! Constrain collision efficiency
                e(i,j,k,m) = e1 + e2 + e3
                if(LimitScavengingEfficiency) e(i,j,k,m) = MIN(e(i,j,k,m),1._dp)
       ...
       DO k=1,nk
          DO j=1,nj
             DO i=1,ni
                ...
                IF(MAXVAL(gamma(i,j,k,:)).gt.EPSILON(1.)) THEN
                DO jj=1,ngh
...
!                   gamma_int=coe*gamma(i,j,k,idx)+omcoe*gamma(i,j,k,idx+1)
  ...
  SUBROUTINE gaulag(x,w,n,alf)
    ...
    DO i = 1, n
       ...
       ELSEIF (i.EQ.2) THEN
         !bloss: Typo fix from Matt Wyant (15.+dp --> 15._dp)
!fix          z = z+(15.+dp+6.25_dp*alf)/(1._dp+.9_dp*alf+2.5_dp*n)
