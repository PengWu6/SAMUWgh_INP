...
if(.not.SFC_FLX_FXD) then
  ...
  if(OCEAN) then
    ...
    if(LES) then ! pwu modified to mimic CEM, so fluxes are spatially variant
       ...
       do j=1,ny
         do i=1,nx
           ...
           if(doFixedWindSpeedForSurfaceFluxes) then
             ...
             ! normal call
            ! pwu changed pres(1) to presi(1)
             call oceflx(presi(1),0.5*(u(i+1,j,1)+u(i,j,1))+ug, &
           ...
           ustar(i,j) = ustar0
           u10arr(i,j) = u10n
           ! modified per v6.11.6
           fluxbu(i,j) = taux0/rhow(1)
           fluxbv(i,j) = tauy0/rhow(1)

          ! below are how fluxbu and fluxbv computed in blossy CEM code
    ...
    if(CEM) then
       ...
       do j=1,ny
         do i=1,nx
           ...
           ! pwu changed pres(1) to presi(1)
           call oceflx(presi(1),0.25*(u(i+1,j-YES3D,1)+u(i,j-YES3D,1)+u(i+1,j,1)+u(i,j,1))+ug, &
                       v(i,j,1)+vg, &
                       0.5*(t(i,j-YES3D,1)+t(i,j,1))-gamaz(1),0.5*(qvs(i,j-YES3D)+qvs(i,j)), &
                       0.5*(t(i,j-YES3D,1)+t(i,j,1)),z(1), &
                       0.5*(sstxy(i,j-YES3D)+sstxy(i,j))+t00, lw_dn_local, sw_dn_local, & !bloss(2021-08): Add radiative fluxes
