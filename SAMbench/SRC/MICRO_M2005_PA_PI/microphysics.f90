...
use vars, only: pres, rho, dtn, w, t, tabs, qv, qcl, qpl, qci, qpi, &
     ...
     sstxy, t00  ! pwu COMBLE-MIP phase 2
     
use module_mp_GRAUPEL, only: GRAUPEL_INIT, M2005MICRO_GRAUPEL, polysvp, &
  arctic_ice_nuclei_Nice, cjhet, mjhet
use micro_params
...
subroutine micro_setparm()
  ...
  integer :: count
   ...
   NAMELIST /MICRO_M2005_PA_PI/ &
      ...
      doabifm,        &     ! use ABIFM (Knopf et al. 2023) for heteogeneous ice formation
      ...
      arctic_ice_nuclei_Nice, &
      cjhet, mjhet, &       ! coefficients for computing heteogeneous freezing efficiency
...
!bloss      do_micro3dout         ! flag to output 3d snapshot for a bunch of micro tendencies
   ...
   arctic_ice_nuclei_Nice = 0.16 ! ice number concentration in # per liter (from MPACE).
   ! SSA aerosol
   cjhet = -3.9346
   mjhet = 26.6132
  ...
  !bloss: read in MICRO_M2005_PA_PI namelist
  read (55,MICRO_M2005_PA_PI,IOSTAT=ios)

  if (ios.ne.0) then
     !namelist error checking
     if(ios.ne.ios_missing_namelist) then
        write(*,*) '****** ERROR: bad specification in MICRO_M2005_PA_PI namelist'
        rewind(55)
        read (55,MICRO_M2005_PA_PI) ! this should give a useful error message
        call task_abort()
     elseif(masterproc) then
        write(*,*) '****************************************************'
        write(*,*) '****** No MICRO_M2005_PA_PI namelist in prm file *********'
        write(*,*) '****************************************************'
     end if
  end if
  close(55)
...
!           write(*,*) '  disable this check in SRC/MICRO_M2005_PA_PI/microphysics.f90'
!           write(*,*) '.... Stopping now ....'
!        end if
!        call task_abort()
!     end if
   ...
   if(masterproc) then
      ...
      write (unit=55,nml=MICRO_M2005_PA_PI,IOSTAT=ios)
      write(55,*) ' '
      close(unit=55)
   end if
...
!bloss(2020-11):        nmicro_fields = nmicro_fields + 3 !brnr add single wet and dry prognostic aerosol modes with 2 moments
  ...
  if(count.ne.nmicro_fields) then
    if(masterproc) write(*,*) 'Error in MICRO_M2005_PA_PI, micro_setparm():'
  ...
  if(do_chunked_energy_budgets.AND.do_aerosol_chunk_budgets.AND.doprogaerosol.AND.(.NOT.isallocatedMKBUDGET)) then
    ...
    allocate(flag_mkbudget(nmicro_fields,n_mkbudget), &
         ...
         STAT=ierr)
    if(ierr.ne.0) STOP 'when allocating mkbudget arrays in MICRO_M2005_PA_PI/microphysics.f90'
    flag_mkbudget(:,:) = 0
     ...
     if(doprogaerosol) then
       ...
       case(0)
         ...
         if((.NOT.allocated(AccumAerosolNumber_snd)).AND.(.NOT.allocated(AccumAerosolMass_snd))) then
           write(*,*) 'Error in MICRO_M2005_PA_PI: initial conditions for accumulation mode'
         ...
         if((MINVAL(AccumAerosolNumber_snd).LT.0.).OR.MINVAL(AccumAerosolMass_snd).LT.0.) then
           write(*,*) 'Error in MICRO_M2005_PA_PI: Input initial condition for accumulation'
         ...
         if(masterproc) then
         write(*,*) 'In MICRO_M2005_PA_PI, using aerinitmode==0.  This means that '
 ...
 if(doprogaerosol) then
  ...
  if(do_chunk_mkbudget) then
    ...
    if(n.ne.n_mkbudget_extra) then
      write(*,*) 'Error in setting up mkbudget_extra output fields in MICRO_M2005_PA_PI/microphysics.f90'
...
! pwu COMBLE-MIP
        do i=1,nx
          do j=1,ny
            Nflux_acc(i,j) = 1.e4 * 0.051682*(u10arr(i,j))**3.41 * &
            (0.3 + 0.1*(max(sstxy(i,j)+t00-273.15, 0.)) &   ! SST in unit of degC
            - 0.0076*(max(sstxy(i,j)+t00-273.15, 0.))**2 &
            + 0.00021*(max(sstxy(i,j)+t00-273.15, 0.))**3) ! units: #/m2/s (pwu, COMBLE-MIP, Jaegle et al. (2011))
...
subroutine micro_write_fields2D(nfields1)
  ...
  if(save2Davg) then
    write(*,*) ' No averaging of aerosol 2D outputs in MICRO_M2005_PA_PI.  FIX THIS LATER IF NEEDED...'
...
subroutine cloudradar_init()
  ...
  if(mod(nstatfrq,nskip_quickbeam).ne.0) then
    if(masterproc) then
      write(*,*) '*************************************************************'
      write(*,*) 'ERROR in MICRO_M2005_PA_PI: '
  ...
  if(masterproc) then
    write(*,*) '*************************************************************'
    write(*,*) 'MICRO_M2005_PA_PI: Cloud radar reflectivity output enabled.'
