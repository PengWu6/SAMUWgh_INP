...
!use micro_params
   ...
   integer ndmax,n,i,j,k,kb,iz,it,jt,kc
...
! MO, 07/05/23: added z_inv_glob
   real tmp, grad, grad_max, z_inv_mean
   integer z_inv_ind
...
!	Read Initial Sounding
   ...
   if(doscamiopdata) then
      ...
      if(MINVAL(zz(1:nzsnd)).lt.0.) then
...
998         format(I4,4F12.4)
   ...
   do k=1,nzm
      ...
      if(z(k).lt.4000.) then ! shallow clouds only
         ...
         grad = (t0(kc)-t0(kb))*tmp
         if(grad_max.lt.grad)then
            grad_max = grad
            z_inv_ind=k
         end if
      end if
...
z_inv_glob = z(z_inv_ind) ! MO, 07/05/23: added z_inv_glob
